...
export function HospitalProvider({ children }: { children: ReactNode }) {
    ...
    const fetchHospitals = () => {
        setLoading(true);
        fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || "http://127.0.0.1:8080"}/api/hospitals/`)
            .then((res) => res.json())
            .then((data) => {
                setHospitals(data);
                if (data.length > 0) {
    ...
    const addHospital = async (hospitalData: Omit<Hospital, "id">) => {
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || "http://127.0.0.1:8080"}/api/hospitals/`, {
                method: "POST",
                headers: {
    ...
    const deleteHospital = async (id: number) => {
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || "http://127.0.0.1:8080"}/api/hospitals/${id}/`, {
                method: "DELETE",
            });
